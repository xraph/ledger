...
func toPlanModel(p *plan.Plan) *planModel {
	...
	metadata, _ := json.Marshal(p.Metadata) //nolint:errcheck // best-effort
...
func fromPlanModel(m *planModel) (*plan.Plan, error) {
	...
	if m.Features != "" {
		_ = json.Unmarshal([]byte(m.Features), &features) //nolint:errcheck // best-effort
	}

	var pricing *plan.Pricing
	if m.Pricing != "" && m.Pricing != "null" {
		pricing = new(plan.Pricing)
		_ = json.Unmarshal([]byte(m.Pricing), pricing) //nolint:errcheck // best-effort
	}
...
func toUsageEventModel(e *meter.UsageEvent) *usageEventModel {
	metadata, _ := json.Marshal(e.Metadata) //nolint:errcheck // best-effort
...
type invoiceModel struct {
	...
	LineItems           string     `grove:"line_items"` // JSON text
...
func toInvoiceModel(inv *invoice.Invoice) *invoiceModel {
	...
	metadata, _ := json.Marshal(inv.Metadata)   //nolint:errcheck // best-effort
...
func fromInvoiceModel(m *invoiceModel) (*invoice.Invoice, error) {
	...
	if m.LineItems != "" {
		_ = json.Unmarshal([]byte(m.LineItems), &lineItems) //nolint:errcheck // best-effort
	}
